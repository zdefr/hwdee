...
let currentFocus;
const LIMIT_OVERFLOW_WIDTH = 1100;
const LIMIT_OVERFLOW_HEIGHT = 600;
...
const MapIndex = () => {
    ...
    const [width, setwidth] = useState(LIMIT_OVERFLOW_WIDTH);
    const [height, setHeight] = useState(LIMIT_OVERFLOW_HEIGHT);

    useEffect(()=>{
        setwidth(document.getElementById('content')?document.getElementById('content').offsetWidth:1150);
        setHeight(document.getElementById('content')?document.getElementById('content').offsetHeight:650)
    },[])

    useEffect(()=>{
        axios({
            method:'GET',
            url:'/jg/rank/DominatingTime',
            timeout:5000
        }).then((res)=>{
            let midata = {
                nodes:[],
            ...
            for(const item of res.data){
                midata.nodes.push({
                    ...
                    childrenNum:item.dominatingTime,
                    level:0,
                    tag:item.videoNumber
                })

                midata.nodes.push({
                    id:item.videoNumber+'name',
                    name:item.name.length>8?item.name.substring(0,10)+'\n'+item.name.substring(10,20):item.name,
                    isLeaf:true,
                    level:2,
                    tags:[
                        item.videoNumber
    ...
    const layoutCfg = {
      ...
      G6.registerBehavior('double-finger-drag-canvas', {
        ...
        onWheel: (ev) => {
          if (ev.ctrlKey) {
            ...
            let ratio = graph.current.getZoom();
            if (ev.wheelDelta > 0) {
              ratio = ratio + ratio * 0.05;
            } else {
              ratio = ratio - ratio * 0.05;
            }
            graph.current.zoomTo(ratio, {
              x: point.x,
              y: point.y,
            });
          } else {
      ...
      function translate(x, y) {
        ...
        /* 获得当前偏移量*/
        const group = graph.current.get('group');
        const bbox = group.getBBox();
        const leftTopPoint = graph.current.getCanvasByPoint(bbox.minX, bbox.minY);
        const rightBottomPoint = graph.current.getCanvasByPoint(bbox.maxX, bbox.maxY);
        ...
        graph.current.translate(-moveX, -moveY);
      }
      ...
      const loadData = (data) => {
        ...
        mapNodeSize(showNodes, 'childrenNum', [120, 200]);
      ...
      useEffect(()=>{
        ...
        graph.current = new G6.Graph({
          ...
          graph.current.on('node:dragstart', (e) => {
          ...
          graph.current.on('node:click', (e) => {
            ...
            if (model.level === 0) {
              ...
              setTimeout(() => {
                ...
                nodeItems.forEach((item) => {
                  graph.current.clearItemStates(item);
                  graph.current.setItemState(item, 'appearing', true);
                });
              }, 400);
            }
          });
            ...
            if (highlighting) {
              ...
              nodeItems.forEach((item) => {
                ...
                if (model.level === 0) {
                  ...
                  graph.current.setItemState(item, 'disappearing', true);
                }
              });
              edgeItems.forEach((item) => {
                graph.current.setItemState(item, 'disappearing', true);
    ...
    return (
        <div ref={box} style={{width:'100%', height:'100%'}} id='mapindex'>
